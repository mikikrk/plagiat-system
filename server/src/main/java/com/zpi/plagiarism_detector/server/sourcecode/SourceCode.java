...
public class SourceCode {
    ...
    /**
     * Uruchomienie procesu por�wnywania plik�w
     */
    public void go() {
    ...
    /**
     * Funkcja zwraca duplikaty kodu w plikach w formacie xml, uwzgl�dniaj�c tylko podany plik
     *
        ...
        try {
            ...
            for (int i = 0; i < duplications.getLength(); i++) {
                ...
                for (int j = 0; j < childs.getLength(); j++) {
                    ...
                    if (childs.item(j).getNodeName() == "codefragment") {
                        fragment1 = childs.item(j).getTextContent();
                    } else if (childs.item(j).getNodeName() == "file") {
                        ...
                        if (path.compareTo(filename) == 0) {
                            ...
                            beginNew = Integer.parseInt(file.getAttributes().getNamedItem("line").getNodeValue());
                        } else {
                            //zdalny plik
                            beginExisting = Integer.parseInt(file.getAttributes().getNamedItem("line").getNodeValue());

                            //odczytanie zdublowanych linni z pliku
                            FileInputStream fs = new FileInputStream(path);
                            BufferedReader br = new BufferedReader(new InputStreamReader(fs));
                            for (int k = 1; k < beginExisting; ++k) {
                                br.readLine();
                            }
                ...
                plagiarisedFragments.put(new PlagiarismFragment(fragment1, beginNew, beginNew + size, size),
                        new PlagiarismFragment(fragment2, beginExisting, beginExisting + size, size)
                );
            }
